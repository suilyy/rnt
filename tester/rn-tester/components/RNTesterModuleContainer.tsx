...
type Props = {
  ...
  example?: any;
  onExampleCardPress?: any;
};

function getExampleTitle(title: any, platform: any) {
  return platform != null ? `${title} (${platform} only)` : title;
}

export default function RNTesterModuleContainer(props: Props): any {
  const { module, example, onExampleCardPress } = props;
  const theme = React.useContext(RNTesterThemeContext);
  const renderExample = (e: any, i: any) => {
    ...
    return module.showIndividualExamples === true ? (
      ...
      <RNTesterBlock key={i} title={getExampleTitle(title, platform)} description={description}>
        <ExampleComponent />
      </RNTesterBlock>
    );
  };
  ...
  const filter = ({ example: e, filterRegex }: any) => filterRegex.test(e.title);
    ...
    <>
      ...
      <View style={styles.examplesContainer}>
        <RNTesterExampleFilter
          ...
          sections={sections}
          filter={filter}
          render={({ filteredSections }) => filteredSections[0].data.map(renderExample)}
        />
      </View>
    </>
  );
...
function Header(props: { description: string; theme: RNTesterTheme; noBottomPadding?: boolean }) {
  return (
    <View
      style={[
        ...
        {
          backgroundColor:
            Platform.OS === 'ios' ? props.theme.SystemBackgroundColor : props.theme.BackgroundColor,
        },
      ]}>

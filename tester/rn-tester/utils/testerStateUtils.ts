...
const filterEmptySections = (examplesList: ExamplesList): any => {
  ...
  sectionKeys.forEach(key => {
    filteredSections[key] = examplesList[key].filter(section => section.data.length > 0);
  });
...
export const getExamplesListWithBookmarksAndRecentlyUsed = ({
  ...
  const recentlyUsedComponents = recentlyUsed.components
    .map(recentComponentKey => components.find(component => component.key === recentComponentKey))
    .filter(Boolean);

  const bookmarkedComponents = components.filter(component => component.isBookmarked);

  const apis = RNTesterList.APIs.map(
    (apiExample): RNTesterModuleInfo => ({
      ...apiExample,
  ...
  const recentlyUsedAPIs = recentlyUsed.apis
    .map(recentAPIKey => apis.find(apiExample => apiExample.key === recentAPIKey))
    .filter(Boolean);

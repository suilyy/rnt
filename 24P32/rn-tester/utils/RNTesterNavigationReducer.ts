import type { RNTesterNavigationState, ComponentList } from '../types/RNTesterTypes';
...
const getUpdatedBookmarks = ({
  ...
  if (updatedBookmarks[exampleType].includes(key)) {
    updatedBookmarks[exampleType] = updatedBookmarks[exampleType].filter(k => k !== key);
  } else {
...
export const RNTesterNavigationReducer = (
  ...
  const { data: { key = null, title = null, exampleType = null, screen = null } = {} } = action;
  ...
  switch (action.type) {
    ...
    case RNTesterNavigationActionsType.BACK_BUTTON_PRESS:
      ...
      return {
        ...state,
        activeModuleExampleKey: null,
        activeModuleKey: state.activeModuleExampleKey != null ? state.activeModuleKey : null,
        activeModuleTitle: state.activeModuleExampleKey != null ? state.activeModuleTitle : null,
      };
